...
test('validate inputs and secrets', async () => {
  ...
  expect(magic_url_inputs.output).toContain(
    `magic_url_route=${mockInputs.magic_url_route}`
  );

  // e2e_prepare
  const e2e_prepare_inputs = getTestResult({
    results,
    name: 'e2e_prepare_inputs',
  });

  expect(e2e_prepare_inputs.output).toContain(
    `e2e_artemis_config_path=${mockInputs.e2e_artemis_config_path}`
  );
  expect(e2e_prepare_inputs.output).toContain(
    `user_count=${JSON.parse(mockInputs.e2e_containers).length}`
  );
  ...
  expect(e2e_run_inputs.output).toContain(
    `e2e_filter_tags=${mockInputs.e2e_filter_tags}`
  );
  expect(e2e_run_inputs.output).toContain(
