...
const localizePath = (path: string) =>
  path.replace(/jupiterone\/.github\//g, '').replace(/@.*/g, '');
...
const getCompositeMockSteps = async ({
  ...
  if (!mockCompositeSteps) {
    const pathToCompositeMocks = resolve(
      cwd(),
      `${pathToCompositeFolder}/mocks.ts`
    );

    if (fs.existsSync(pathToCompositeMocks)) {
      const compositeMocks = await import(pathToCompositeMocks);
      mockCompositeSteps = Object.values(
        compositeMocks
      ).flat() as unknown as MockCompositeSteps;
    }
  }
...
const updateCompositeSteps = ({
  ...
  mockCompositeSteps?.forEach((mockStep) => {
    const step = actionYaml?.runs?.steps
      .filter((step) => step?.name === mockStep?.name)
      .shift();
...
const updateStepInJob = async ({
  ...
  const mockJobStep: MockStep = mockSteps[key]
